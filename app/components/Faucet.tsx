...
import { mintToken, deployAccount, deployFaucet } from "@/lib/webClient";
...
export default function Faucet({
  ...
  const handleDeployFaucet = async () => {
    ...
    try {
      const faucetId = await deployFaucet(
        newFaucet.symbol,
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
            />
          </div>
          ...
          <button
            ...
            disabled={!selectedAccount || isDeployingFaucet}
      ...
      <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={handleMint}
            disabled={
              !selectedAccount || !selectedFaucet || !amount || isLoading
            }
            className="w-full px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isLoading ? (
